        ...
        for (key, table) in providers.iter() {
            ...
            rows.push(RescuedRow {
                provider: crate::store::Provider {
                    ...
                    name: key.to_string(),
                    base_url,
                    // auth.json holds one shared key; only the active row
                    // can claim it. Login-based rows don't carry a key.
...
"#,
    ...
    fn rescue_imports_custom_provider_tables() {
        ...
        // plus a second inactive table. Both are adopted; only the active
        // one carries the shared auth.json key, model, and catalog.
        let (_td, paths) = setup();
        init_codex(&paths);
        fs::write(
            paths.codex_dir.join("config.toml"),
...
"#,
        ...
        // requires_openai_auth: ChatGPT login owns auth; no key claimed.
        assert_eq!(agate.provider.api_key, "");
