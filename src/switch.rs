...
pub fn resolve<'a>(store: &'a Store, query: &str, app: Option<AppId>) -> Result<&'a Provider> {
    ...
    match matches.as_slice() {
        ...
        many => {
            let list = many
                .iter()
                .map(|p| format!("{} ({})", p.id, p.app))
                .collect::<Vec<_>>()
                .join(", ");
            anyhow::bail!("ambiguous provider '{query}': {list}");
        }
    }
}
...
pub fn add_provider(paths: &Paths, store: &mut Store, opts: AddOpts) -> Result<String> {
    ...
    let id = match opts.id {
        Some(id) => {
            validate_explicit_id(&id, store)?;
            id
        }
        None => generate_id(&opts.name, store)?,
    };
...
fn use_provider_inner(
    ...
    persist(
        ...
        paths,
        after_live,
        &format!("re-run aimux use {}", provider.id),
    )?;
    Ok((provider.id, outcome))
}
...
fn validate_explicit_id(id: &str, store: &Store) -> Result<()> {
    if !is_valid_id(id) {
        anyhow::bail!("invalid --id '{id}': must match [a-z0-9]+(-[a-z0-9]+)*");
    }
    if store.providers.contains_key(id) {
        anyhow::bail!("id '{id}' already exists");
    }
    Ok(())
}
...
fn generate_id(name: &str, store: &Store) -> Result<String> {
    ...
    if base.is_empty() {
        anyhow::bail!("could not generate id from name '{name}'; pass --id");
    }
    let mut n = 2;
...
mod tests {
    ...
    fn official_row_cannot_be_deleted_or_edited() {
        ...
        let id = use_provider(&paths, &mut store, "claude-official", None).unwrap();
        assert_eq!(id, "claude-official");
    }
    ...
    #[test]
    fn empty_sanitised_name_requires_id() {
        let (_td, paths, mut store) = setup();
        let err = add_provider(
            &paths,
            &mut store,
        ...
        .unwrap_err();
        assert!(err.to_string().contains("pass --id"));
    ...
    fn resolve_exact_id_and_name_substring() {
        ...
        assert_eq!(resolve(&store, "OtherP", None).unwrap().id, "other");
    ...
    fn use_opencode_writes_live_and_sets_current() {
        ...
        assert_eq!(store.current[&AppId::OpenCode], id);
