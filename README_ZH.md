...
**公共配置**在添加/编辑表单里，是该供应商自己的 JSON。上面是内置勾选项（Claude 隐藏署名 / Teammates / Tool Search / 思考强度 / 禁用自动升级；Codex Goal mode），勾选会写进 JSON。表单里的「应用公共配置」，或 `aimux add/edit --apply-snippet`。片段先合并，自有字段后写，后者赢。`aimux snippet <name>` 查看/设置/清除该供应商的 JSON。
